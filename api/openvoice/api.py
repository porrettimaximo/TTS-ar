...
class ToneColorConverter(OpenVoiceBaseClass):
    ...
    def extract_se(self, ref_wav_list, se_save_path=None):
        ...
        for item in ref_wav_list:
            ...
            if sr != hps.data.sampling_rate:
                wav = torchaudio.transforms.Resample(sr, hps.data.sampling_rate)(wav)
    ...
    def convert(self, audio_src, src_se, tgt_se, output_path=None, tau=0.3, message="default"):
        import torchaudio
        hps = self.hps
        ...
        with torch.no_grad():
            ...
            audio_out = self.model.voice_conversion(spec, spec_lengths, sid_src=src_se, sid_tgt=tgt_se, tau=tau)[0][0, 0].data.cpu().float()
            
            # Normalizar para evitar clipping
            max_v = torch.max(torch.abs(audio_out))
            if max_v > 1.0:
                audio_out = audio_out / max_v * 0.95

            audio_np = audio_out.numpy()

            if output_path is None:
                return audio_out.unsqueeze(0), hps.data.sampling_rate
            else:
                soundfile.write(output_path, audio_np, hps.data.sampling_rate)
                return audio_out.unsqueeze(0), hps.data.sampling_rate
